...
public class TitleColor extends Functions
{
	public void list(String[] param)
	{
		...
		{
			String color = Config.SERVICES_CHANGE_Title_COLOR_LIST[i];
			block = template;
		...
		double count = Math.ceil((double) Config.SERVICES_CHANGE_Title_COLOR_LIST.length / (double) perpage); // Use rounding to get the last page with the rest!
		int inline = 1;
		String navigation = "";
		for (int i = 1; i <= count; i++)
		{
			if (i == page)
			{
				navigation += "<td width=25 align=center valign=top><button value=\"[" + i + "]\" action=\"bypass -h scripts_services.TitleColor:list " + i + "\" width=32 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>";
			}
			else
			{
				navigation += "<td width=25 align=center valign=top><button value=\"" + i + "\" action=\"bypass -h scripts_services.TitleColor:list " + i + "\" width=32 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td>";
	...
	public void change(String[] param)
	{
		...
		if (player.getInventory().destroyItemByItemId(Config.SERVICES_CHANGE_Title_COLOR_ITEM, Config.SERVICES_CHANGE_Title_COLOR_PRICE, "removed price"))
		{
			player.setTitleColor(Integer.decode("0x" + param[0]));
			player.broadcastUserInfo(true);
			player.sendMessage("Your title color was changed!");
		}
