	...
	public void onBypassFeedback(Player player, String command)
	{
			...
			else if (command.equalsIgnoreCase("warp"))
			{
				...
				if (ItemFunctions.removeItem(player, 13797, 1, true, "DeadTumorInstance") > 0 && player.isInParty())
				{
					...
					if (loc != null)
					{
						...
						{
							p.sendPacket(new ExShowScreenMessage(NpcString.S1S_PARTY_HAS_MOVED_TO_A_DIFFERENT_LOCATION_THROUGH_THE_CRACK_IN_THE_TUMOR, 8000, ExShowScreenMessage.ScreenMessageAlign.MIDDLE_CENTER,
										false, 1, -1, false, player.getParty().getLeader().getName()));
						}
		...
		else if (getReflection().getInstancedZoneId() == 121)
		{
			...
			else if (command.equalsIgnoreCase("warpechmus"))
			{
				...
				for (Player p : getReflection().getPlayers())
				{
					p.sendPacket(new ExShowScreenMessage(NpcString.S1S_PARTY_HAS_MOVED_TO_A_DIFFERENT_LOCATION_THROUGH_THE_CRACK_IN_THE_TUMOR, 8000, ExShowScreenMessage.ScreenMessageAlign.MIDDLE_CENTER, false,
								1, -1, false, player.getParty().getLeader().getName()));
				}
				for (Player p : player.getParty().getMembers())
				{
			...
			else if (command.equalsIgnoreCase("reenterechmus"))
			{
				...
				if (ItemFunctions.removeItem(player, 13797, 3, true, "DeadTumorInstance") >= 3 && player.isInParty())
				{
					for (Player p : getReflection().getPlayers())
					{
						p.sendPacket(new ExShowScreenMessage(NpcString.S1S_PARTY_HAS_ENTERED_THE_CHAMBER_OF_EKIMUS_THROUGH_THE_CRACK_IN_THE_TUMOR, 8000, ExShowScreenMessage.ScreenMessageAlign.MIDDLE_CENTER,
									false, 1, -1, false, player.getParty().getLeader().getName()));
					}
